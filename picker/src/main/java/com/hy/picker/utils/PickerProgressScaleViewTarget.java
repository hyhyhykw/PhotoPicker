...
public class PickerProgressScaleViewTarget extends CustomViewTarget<ProgressScaleImageView, File> {
    ...
    public void onResourceReady(@NonNull File resource, @Nullable Transition<? super File> transition) {
        ...
        new InitTask(this).execute(path);
    }


    private static class InitTask extends AsyncTask<String, Integer, _ScaleBean> {
        private WeakReference<PickerProgressScaleViewTarget> mReference;

        InitTask(PickerProgressScaleViewTarget progressScaleViewTarget) {
...
//            view.getProgressView().toNext(() -> view.getProgressView().cancel());
            ...
            view.getScaleImageView().setMaxScale(scaleBean.scale + 2.0f);//最大显示比例


            view.getScaleImageView().setImage(ImageSource.bitmap(scaleBean.mBitmap),
